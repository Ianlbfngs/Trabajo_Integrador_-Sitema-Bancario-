		...
		<div class="flex-1 p-6">
			...
			<header class="border-b pb-4">
				<h2 class="text-2xl font-bold">
					Editar de Cuenta
					<%
					request.getAttribute("NumeroCuenta");
				%>
				</h2>
			</header>
			...
			<div class="bg-gray-100 p-6 rounded-md shadow-md">
				...
				<%
					...
					if (request.getAttribute("mensaje") != null) {
						...
						if (mensaje.equals("*Cuenta actualizada correctamente*")) {
				...
				<!-- Formulario -->
				<form name="form_Editar_cuenta" action="ServletControladores"
					onsubmit="return validarFormulario(2)" method="post"
					class="space-y-4">
					<%
						Cuenta cuenta = (Cuenta) request.getAttribute("CuentaEditar");
					%>
					<!-- Numero de cuenta -->
					<div>
						<label for="numcue" class="block mb-2 font-medium">Numero
							De Cuenta:</label> <input type="text" id="num" name="numCuenta"
							minlength="5" maxlength="5" readonly
							onkeypress="return (event.charCode >= 48 && event.charCode <= 57)"
							value="<%=cuenta != null ? cuenta.getNumeroDecuenta_Cuen() : ""%>"
					...
					<div>
						<label for="dni" class="block mb-2 font-medium">DNI:</label> <input
							...
							readonly
							onkeypress="return (event.charCode >= 48 && event.charCode <= 57)"
							value="<%=cuenta != null ? cuenta.getDniCliente_Cuen() : ""%>"
							class="w-full p-2 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
					...
					<!-- Tipo de Cuenta -->
					<div>
						<label for="id_tipodeCuetna" class="block mb-2 font-medium">Tipo
							de cuenta:</label> <select id="tipo_de_cuenta_alta"
							name="id_tipodeCuetna" required
							class="w-full p-2 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
							...
							%>
							<option
								<%=(TCuenta.getIdTipoDeCuenta() == cuenta.getTipoCuenta_Cuen().getIdTipoDeCuenta())
								? "selected"
								: ""%>
								value="<%=TCuenta.getIdTipoDeCuenta()%>">
					...
					<div>
						<label for="CBU" class="block mb-2 font-medium">CBU:</label> <input
							...
							readonly
							onkeypress="return (event.charCode >= 48 && event.charCode <= 57)"
							value="<%=cuenta != null ? cuenta.getCBU_Cuen() : ""%>"
							class="w-full p-2 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
					</div>
					<input type="date" id="fecha_creacion" name="fecha_creacion"
						value="<%=cuenta != null ? cuenta.getFechaCreacion_Cuen() : ""%>"
						required hidden> <input type="text" id="saldo_cuenta"
						name="saldo_cuenta"
						value="<%=cuenta != null ? cuenta.getSaldo_Cuen() : ""%>"
						required hidden>
					<!-- Botón -->
					<div class="text-center">
						<button type="submit" name="btnModficiarCuenta"
						...
						<a href="ServletControladores?Pagina=listarCuentas" type="submit"
